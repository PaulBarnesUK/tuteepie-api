...
class TutorCanGetStudentTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test()
    {
        ...
        $createUserResponse = $this->withHeaders(['Authorization' => $authHeader])
        ...
        $studentId = $createUserResponse->baseResponse->original->data->id;

        $this->json('GET', "/api/{$this->apiVersion}/tutors/{$tutor->id}/students/{$studentId}")
            ->assertSuccessful()
            ->assertJsonStructure([
                'data' => [
                    'type',
